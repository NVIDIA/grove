...
func TestExtractScalingGroupNameFromPCSG(t *testing.T) {
	tests := []struct {
		name           string
		pcsgName       string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := ExtractScalingGroupNameFromPCSG(tt.pcsgName, tt.pgsNameReplica)
			if result != tt.expected {
				t.Errorf("ExtractScalingGroupNameFromPCSG() = %q, expected %q", result, tt.expected)
			}
		})
	}
}

func TestExtractScalingGroupNameFromPCSG_Consistency(t *testing.T) {
	// Test that ExtractScalingGroupNameFromPCSG is the inverse of GeneratePodCliqueScalingGroupName
	testCases := []struct {
		pgsNameReplica   ResourceNameReplica
	...
	for i, tc := range testCases {
		t.Run("consistency_test", func(t *testing.T) {
			...
			extractedScalingGroupName := ExtractScalingGroupNameFromPCSG(generatedPCSGName, tc.pgsNameReplica)

...
func TestClusterTopologyCRDValidation(t *testing.T) {
	...
	t.Run("reject invalid domain", func(t *testing.T) {
		...
		err := k8sClient.Create(context.Background(), ct)
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "supported values")
	})
	...
	t.Run("reject empty key", func(t *testing.T) {
		...
		err := k8sClient.Create(context.Background(), ct)
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "spec.levels[0].key")
	})

	t.Run("reject key too long", func(t *testing.T) {
		// Create a key longer than 63 characters (MaxLength validation)
		longKey := "this-is-a-very-long-key-that-exceeds-the-maximum-allowed-length-of-63-characters"
		ct := &grovecorev1alpha1.ClusterTopology{
	...
	t.Run("reject empty levels array", func(t *testing.T) {
		ct := &grovecorev1alpha1.ClusterTopology{
		...
		err := k8sClient.Create(context.Background(), ct)
		assert.Error(t, err)
		assert.Contains(t, err.Error(), "spec.levels")
	})

	t.Run("reject too many levels", func(t *testing.T) {
		// Create 8 levels (exceeds MaxItems=7)
		ct := &grovecorev1alpha1.ClusterTopology{
	...
	t.Run("reject missing domain", func(t *testing.T) {
		ct := &grovecorev1alpha1.ClusterTopology{

...
// TestValidateNetworkPackGroupConfigs tests validation of network pack group configurations.
func TestValidateNetworkPackGroupConfigs(t *testing.T) {
	tests := []struct {
		// name identifies this test case
		name string
		// pcs is the PodCliqueSet to validate
		...
		{
			name: "duplicate cliques in network pack groups returns error",
			pcs: createPodCliqueSetWithNetworkPackGroups([]*grovecorev1alpha1.PodCliqueTemplateSpec{
				createDummyPodCliqueTemplate("clique1"),
				createDummyPodCliqueTemplate("clique2"),
			}, []grovecorev1alpha1.NetworkPackGroupConfig{
				{
			...
			expectError:   true,
			errorContains: "cannot belong to more than one",
		},
		{
			name: "network pack group with cliques not in scaling group returns error",
			pcs: createPodCliqueSetWithNetworkPackGroups([]*grovecorev1alpha1.PodCliqueTemplateSpec{
				createDummyPodCliqueTemplate("clique1"),
				createDummyPodCliqueTemplate("clique2"),
				createDummyPodCliqueTemplate("clique3"),
			}, []grovecorev1alpha1.NetworkPackGroupConfig{
				{
			...
			expectError:   true,
			errorContains: "cannot partially include",
		},
		{
			name: "valid network pack groups pass validation",
			pcs: createPodCliqueSetWithNetworkPackGroups([]*grovecorev1alpha1.PodCliqueTemplateSpec{
				createDummyPodCliqueTemplate("clique1"),
				createDummyPodCliqueTemplate("clique2"),
			}, []grovecorev1alpha1.NetworkPackGroupConfig{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			validator := newPCSValidator(tt.pcs, "Create")
			fldPath := field.NewPath("spec", "template", "schedulingPolicyConfig", "networkPackGroupConfigs")
			errs := validator.validateNetworkPackGroupConfigs(fldPath)
			if tt.expectError {
				assert.NotEmpty(t, errs)
				if tt.errorContains != "" {
					errorString := ""
...
// Helper function to create a PodCliqueSet with network pack groups for testing
func createPodCliqueSetWithNetworkPackGroups(
	cliques []*grovecorev1alpha1.PodCliqueTemplateSpec,
	networkPackGroups []grovecorev1alpha1.NetworkPackGroupConfig,
	scalingGroups []grovecorev1alpha1.PodCliqueScalingGroupConfig,
) *grovecorev1alpha1.PodCliqueSet {
	pcs := createDummyPodCliqueSet("test-pcs")
	pcs.Spec.Template.Cliques = cliques
	pcs.Spec.Template.SchedulingPolicyConfig = &grovecorev1alpha1.SchedulingPolicyConfig{
		NetworkPackGroupConfigs: networkPackGroups,
	}
	pcs.Spec.Template.PodCliqueScalingGroupConfigs = scalingGroups
	return pcs
}
